...
get_plot_changes <- function(sphere_){
  ...
  get_hashes_3(sphere_) %>% 
    ...
    geom_point() +
    theme_b03_base +theme_b03_base_typo_static + theme(legend.position = "none", panel.grid.major.y = element_line(color = "#dddddd", linewidth = .2))
}
...
## hier fehlen alle seiten ohne form-findings
  ...
  df_hashes_2 %>% 
    ...
    ggplot(., aes(x = crawl_date, y = site_subdomain, color = as.character(nr_unique_hashes))) +
    geom_point() +
    theme_b03_base +theme_b03_base_typo_static + theme(legend.position = "none")
