...
Was hier auffällt: die Snippetliste findet in den Tags viel mehr, als über die plain text Suche. Das ist ein false friend, wahrscheinlich wurden die Spuren aus der plain-text-Suche reduziert auf eine Spur, selbst wenn es mehrere in einer Seite gegeben hätte. Bei dieser Methode liefert die Information darüber, wie oft ein Snippet gefunden wurde keinen Mehrwert, im Gegensatz zur Suche in den Tags, denn daraus lassen sich Rückschlüsse auf Programmierpraktiken schließen und diese Kenntnis können wiederum hilfreich sein für den Übersetzungprozess den Wandel von Kommentaroptionen zu visualisieren. 

Außerdem waren die Snippetliste(n) nicht die gleichen. Für die Suche in den Tags, wurde die Liste stark vereinfacht. Wobei es im internationalen Sprachraum fast auf die Zahl genau gleich viele Funde sind. Hier ist also noch einmal Recherche notwendig, was da in der Datenbank ist und ob irgendeine andere Unregelmäßigkeit vorliegt.
...
# findings_tags <- df_snippets_de %>% filter(search_method == "tag", search_type == "snippet_list") 
# findings_plain_text <- df_snippets_de %>% filter(search_method == "plain_text", search_type == "snippet_list")
...
# findings_tags_world <- df_snippets_world %>% filter(search_method == "tag", search_type == "snippet_list") 
# findings_plain_text_world <- df_snippets_world %>% filter(search_method == "plain_text", search_type == "snippet_list")
...
# findings_tags_nl <- df_snippets_nl %>% filter(search_method == "tag", search_type == "snippet_list") 
# findings_plain_text_nl <- df_snippets_nl %>% filter(search_method == "plain_text", search_type == "snippet_list")
