---
title: "Zweite Datenbank Tabelle: tags"
execute:
  eval: false
---
...
Diese Schnipsel lesen die Dateien ein, die von dem Script `parsing-html.R` erzeugt wurden. Für jeden Tag und jede Sphere gibt es einen eigenen Datenordner. Hier wird außerdem eine kurze Prüfung des jeweiligen Datensatzes vorgenommen, bevor der Abschnitt in die Datenbank geladen wird.

Inzwischen laufen die Abschnitte nicht mehr durch. Die einzelen Datensätze sind nur noch auf einer exterenen Festplatte gespeichert und nicht mehr auf dem Computer, auf dem die Datenbanktabellen erzeugt wurden. 

```{r}
# Sinn der folgenden Zeilen unklar
...
list_of_files <- list.files(path = paste0("../../../data/1-parsing/doctype/",CURRENT_SPHERE,"/"), recursive = FALSE, pattern = "\\.csv$", full.names = TRUE)
df_tags <- read_csv(list_of_files, id = "file_name")#, col_names = header)
# in den Daten sind keine Duplikate
df_tags %>% distinct() %>% nrow()
# es sind mehr Seiten geparsed worden, als of interest sind
...
df_tags_db <- df_tags %>% 
  ...
  select(-file_name, -`...1`, -doctype)  

# save(df_tags_db, file = (paste0("../../data/1-parsing/doctype/",CURRENT_SPHERE,"/db_doctype.RData")))
# load(file = (paste0("../../data/1-parsing/doctype/",CURRENT_SPHERE,"/db_doctype.RData")))
dbWriteTable(conn = con, name = "tags", df_tags_db, append = TRUE)

```

## Appendix: plain text / snippet table
...
df_snippets <- read_csv(list_of_files, id = "file_name", col_names = header)

df_snippets_db <- df_snippets %>% 
  # filter(site %in% sites_of_interest) %>% # already done before parsing
  mutate(sphere = CURRENT_SPHERE) %>% 
  select(-file_name)  


dbWriteTable(conn = con, name = "snippets", df_snippets_db, append = TRUE)
