...
# get_data_snippets_plain_text_2 <- function(sphere_){
...
get_data_snippets_plain_text <- function(sphere_){
  df <- dbGetQuery(conn = con, paste0("SELECT DISTINCT t.site, t.snippet, s.sphere FROM sites s INNER JOIN snippets t ON s.sha1 = t.site WHERE t.detected = 1 AND s.sphere ='", sphere_,"' AND s.of_interest = TRUE"), check_interrupts = TRUE) %>% 
  mutate(
    search_topic = "comment",
    search_method = "plain_text",
    search_type = "snippet_list") %>% 
...
df_test <- dbGetQuery(conn = con, paste0("SELECT DISTINCT t.site as sha1, t.trace, t.tag, t.search_type, t.search_method, t.comment, t.sphere, t.traces_id FROM traces t WHERE LOWER(t.trace) = ('coral')"), check_interrupts = TRUE)

# DELETE FROM traces WHERE LOWER(t.trace) = ('coral')

dbWriteTable(conn = con, name = "traces", better_trace, append = TRUE)
...
dbWriteTable(conn = con, name = "traces", df_snippets_plain_de, append = TRUE)
...
dbWriteTable(conn = con, name = "traces", df_snippets_plain_world, append = TRUE)
```
...
dbWriteTable(conn = con, name = "traces", df_snippets_plain_nl, append = TRUE)
# dbDisconnect(con)
