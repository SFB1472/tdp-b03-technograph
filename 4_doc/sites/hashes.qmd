...
get_context_data <- function(sphere_){
  df <- dbGetQuery(conn = con, paste0("SELECT tc.sha1, tc.tag, tc.attr, tc.value, tc.context_path, tc.context_path_cleaned, tc.id_sha1_group, tc.traces_context_id FROM traces_context tc  WHERE tc.sphere = '", sphere_, "'"))
  
}

get_context_data_ <- function(sphere_){
  df <- dbGetQuery(conn = con, paste0("SELECT tc.attr, tc.value, COUNT(*) as counted FROM traces_context tc  WHERE tc.sphere = '", sphere_, "' GROUP BY tc.attr, tc.value"))
  
}

# testing <- get_context_data_("German")

get_context_data_partiell <- function(sphere_, special_id){
  df <- dbGetQuery(conn = con, paste0("SELECT tc.sha1, tc.tag, tc.attr, tc.value, tc.context_path, tc.id_sha1_group, tc.traces_context_id FROM traces_context tc  WHERE tc.sphere = '", sphere_, "' AND id_sha1_group = '", special_id,"'"))
  
}

get_context_data_partiell_ids <- function(sphere_, special_ids){
  df <- dbGetQuery(conn = con, paste0("SELECT tc.sha1, tc.tag, tc.attr, tc.value, tc.context_path, tc.id_sha1_group, tc.traces_context_id FROM traces_context tc  WHERE tc.sphere = '", sphere_, "' AND id_sha1_group ~ '", special_ids,"'"))
  
}


# get_context_data <- function(sphere_, site_){
#   df <- dbGetQuery(conn = con, paste0("SELECT s.site, tc.sha1, tc.tag, tc.attr, tc.value, tc.context_path, tc.id_sha1_group, tc.traces_context_id FROM site s INNER JOIN traces_context tc ON s.sha1 = tc.sha1 WHERE tc.sphere = '", sphere_, "' AND s.site = '", site,"'"))
#   
# }

get_context_info_site <- function(sphere_, site_){
  df <- dbGetQuery(conn = con, paste0("SELECT DISTINCT t.sha1, t.context_path_cleaned, t.context_path, t.id_sha1_group FROM sites s INNER JOIN  traces_context t ON s.sha1 = t.sha1 WHERE t.sphere = '", sphere_,"' AND s.site ='", site_,"'"))
}
...
get_hashes_cleaned <- function(sphere){
  df <- dbGetQuery(conn = con, paste0("SELECT s.crawl_date, s.site, s.url, s.sha1, tc.hashed_context, tc.id_sha1_group FROM sites s INNER JOIN context_hashed tc ON tc.sha1 = s.sha1 WHERE s.sphere = '", sphere, "' AND tc.iteration ='cleaned_whitelist' AND s.of_interest = TRUE ORDER BY s.crawl_date"))
 df_return <- make_df(df)
}

# testing <- get_hashes_cleaned("German")
...
Inzwischen denke ich, sollten die Werte der Attribute schon wieder in den Hash eingehen, allerdings ist dann eine notwendig die Daten ein wenig zu cleanen. Denn wie in dem vorherigen Dokument zu den Hashes geschildert, ist es tatsächlich so, dass sich hier oftmals ids finden. Manchmal beziehen sie sich auf die Texte, die kommentiert werden können, manchmal auf User (div data-userid), Autoren oder Kommentare haben selbst ids (div id': 'comments-40383508 ´; article data-comments-block-id). Auch störend sind beispielsweise img-descriptions, die durch den Context jetzt gelegentlich dabei sind (div data-video-description, aber nur 33 Vorkommen). 
...
### Varianten Hashes zu berechnen

#### pragmatic

Die pragmatische Lösung hier ist für die Hashes lediglich die Struktur der Bereiche zu beachten. Es werden hier die Tags und die Namen der Attribute berücksichtigt, die Werte der Attribute und der Text in der Seite nicht. 

Nachteil der Variante ist, dass als individuelle Faktoren wie der Name einer Klasse oder ID rausfallen und somit kleine Veränderungen in einem Like-Button unsichtbar werden. 

In der Datenbanktablle sind diese Hashes in der `iteration`-Spalte markiert mit "pragmatic".

#### cleaned whitelist

Diese Variante erweitert den pragmatischen ein wenig. Hier werden Werte von Attributen in den Hash mit einbezogen, aber nur von solchen, die für den Tag identitätsstiftend sind, also die Werte von Klassen und IDs, alle anderen bleiben leer. Außerdem werden die Bereiche im Umfang reduziert, so dass es jedes Vorkommen nur noch einmal gibt. Immer wieder kommt es vor, das Kommentare kodiert sind und sich die Struktur in der Seite wiederholt, wird diese dann mitcodiert, ändert sich ein Hash von Seite zu Seite womöglich nur deshalb, weil es mehr oder weniger Kommentare gibt. 

#### cleaned countbased

Ein erster Vereinfachungsversuch versuchte entlang solcher Attribut-Wert-Kombination vorzugehen, die besonders individuell sind. Werden diese Kombinationen gezählt, kommen einmalige Kombinationen mit Abstand am häufigsten vor. Das liegt u.a. daran, dass in Attributen oft Verknüpfungen auf den aktuellen Artikel oder einen kommentierenden User in Form von IDs enthalten sind. Diese Informationen sollten bei dem countbased-Ansatz vereinfacht werden, um dadurch möglichst viele Informationen im Hash kodieren zu können, ohne das dieser zu sensibel reagiert.

Im Vergleich mit den beiden Erstgenannten (siehe Grafik weiter unten), schneidet dieser Ansatz so schlecht ab, dass der Aufwand nur für die deutsche Sphere durchgeführt wurde. 
...
df_context_de_ <- get_context_data_("German")
...
duplicate_attr_values_de <- df_context_de_ %>%
  # reframe(counted = n(), .by = c(attr, value)) %>% 
  arrange(desc(counted)) 

# rm(df_context_de)

DT::datatable(duplicate_attr_values_de %>% head(50))
...
df_context_world_ <- get_context_data_("World")

# ## Welche Kombinationen von Attributen und Werten kommen wie häufig vor?
# # interessant werden hier vor allem die einstelligen Einträge
duplicate_attr_values_world <- df_context_world_ %>%
  # reframe(counted = n(), .by = c(attr, value)) %>% 
  arrange(desc(counted)) 

# rm(df_context_de)

DT::datatable(duplicate_attr_values_world %>% head(50))
...
histo_tbl <- duplicate_attr_values_de %>% 
...
histo_tbl <- duplicate_attr_values_world %>% 
  reframe(histogram = n(), .by = counted) %>% 
  arrange(desc(histogram)) 
...
individual_attrs_de <- duplicate_attr_values_de %>%
...
most_individual_attrs_de <- duplicate_attr_values_de %>%
  filter(counted == 1) %>% #View()
  reframe(counted = n(), .by = attr) %>% #View()
  arrange(desc(counted))

DT::datatable(most_individual_attrs_de %>% head(50))
...
# ## auf einen Blick: welche Werte haben die einmal vorkommenden Attribute
individual_attrs_world <- duplicate_attr_values_world %>%
  filter(counted == 1) #%>% #View()
  # head(50)
...
## und welche Attribute haben die meisten unterschiedlichen Werte, denn ab denen würde ich mit dem Filtern ansetzen.
most_individual_attrs_world <- duplicate_attr_values_world %>%
  filter(counted == 1) %>% #View()
  reframe(counted = n(), .by = attr) %>% #View()
  arrange(desc(counted))

DT::datatable(most_individual_attrs_world %>% head(50))
...
DT::datatable(individual_attrs_de %>% head(50))
...
Datenstand: Für die errechneten Hashes gibt es nun wieder eine neue Datenbanktabelle. Die Hashvarianten - pragmatisch, cleaned_countbased und cleaned-whitespace - liegen in der gleichen Tabelle, unterscheidbar über eine Spalte die `iteration` heißt. 
...
plot_data <- df_hashed_site %>% 
  ...
  filter(site_subdomain == "lvz_www", iteration == "cleaned_whitelist") %>% 
  left_join(., df_context_info_site)#, by = c("sha1", "id_sha1_group"))

plot <- plot_data %>% 
  ggplot(., aes(x = crawl_date, y = context_path_cleaned, color = as.character(nr_unique_hashes), tooltip = paste0("context: ", context_path, " hashnr: ", nr_unique_hashes))) +
...
df_hashes_cleaned <- get_hashes_cleaned("German")

plot <- df_hashes_cleaned %>%
  mutate(subdomain = domain(url) %>% suffix_extract(.) %>% select(subdomain) %>% pull(.),
           site_subdomain = paste(site, subdomain, sep = "_")) %>% 
  left_join(., df_context_de, by = c("sha1", "id_sha1_group")) %>% 
  filter(site_subdomain %in% exclude_from_plotting$site_subdomain) %>% # View()
  ggplot(., aes(x = crawl_date, y = context_path_cleaned, color = as.character(nr_unique_hashes))) +
  geom_point ()+
...
get_hashes_counted <- function(sphere_){
  df <- dbGetQuery(conn = con, paste0("SELECT s.sha1, s.site, s.url, c.hashed_context, c.iteration, c.id_sha1_group FROM sites s INNER JOIN context_hashed c ON s.sha1 = c.sha1 WHERE s.of_interest = TRUE AND c.sphere ='",sphere_,"'")) %>% 
...
df_hashes_counted_de <- get_hashes_counted("German")

df_hashes_counted_de %>% 
...
df_hashes_counted_world <- get_hashes_counted("World")

df_hashes_counted_world %>% 
  # select(-hashed_context) %>% 
  filter(sum_hashes>55) %>% 
  mutate(sorting = sum(different_hashes), .by = c(site_subdomain, site, iteration)) %>% #View()
  filter(iteration == "cleaned_whitelist") %>% #View()
  ggplot(., aes(x = sum_hashes, y = reorder(site_subdomain, sum_hashes), label = sum_hashes)) +
  geom_col() + 
  geom_text(hjust = 0) +
  scale_x_continuous(expand = c(0,NA), limits = c(0, 80000)) +
  coord_cartesian(clip = "off") +
  theme_b03_base + theme_b03_base_typo_static + theme(axis.text.x = element_blank())
...
df_hashes_counted_world %>% 
  ...
  geom_col(position = "dodge")+ 
  geom_text(position = position_dodge(width = .9), hjust = -0.1, vjust = 0.5, size = 3) +
  scale_x_continuous(expand = c(0,NA), limits = c(0, 3600)) +
  guides(fill = guide_legend(title.position = "top", override.aes = list(shape = 22))) +
  coord_cartesian(clip = "off") +
  theme_b03_base + theme_b03_base_typo_static  + theme(axis.text.x = element_blank())
...
hashes_balance <- df_hashes_counted_de %>% 
...
hashes_balance <- df_hashes_counted_world %>% 
  mutate(balance = round(different_hashes/sum_hashes, digits = 2), .by = c(site_subdomain, site, iteration)) %>% 
  filter(sum_hashes>55)
...
df_handelsblatt <- dbGetQuery(conn = con, paste0("SELECT s.sha1, s.site, s.url, tc.id_sha1_group, tc. tag, tc.attr, tc.value_cleaned_whitelist, tc.context_path, tc.context_path_cleaned FROM sites s INNER JOIN traces_context tc ON s.sha1 = tc.sha1 WHERE s.site = 'handelsblatt'")) %>% 
  mutate(subdomain = urltools::domain(url) %>% suffix_extract(.) %>% select(subdomain) %>% pull(.),
                   site_subdomain = paste(site, subdomain, sep = "_") %>% as.character(.))
