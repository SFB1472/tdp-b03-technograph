...
get_sites_per_day <- function(sphere){
  df_sites_per_day <- dbGetQuery(conn = con, paste0("SELECT s.crawl_date, COUNT(s.sha1) as sites_per_day FROM sites s  WHERE s.sphere = '", sphere, "' AND s.of_interest = true GROUP BY s.crawl_date"))
}

get_form_info <- function(sphere, tag){
  
  df <- dbGetQuery(conn = con, paste0("SELECT DISTINCT t.site, t.tag, t.group, s.crawl_date, s.sha1, s.sphere FROM sites s INNER JOIN tags t ON t.site = s.sha1 WHERE s.sphere ='", sphere, "' AND t.tag = '", tag, "' AND s.of_interest = true")) %>% 
  group_by(crawl_date) %>% 
  summarise(nr_tags = n()) %>% 
...
```{r}
get_sites_with_form <- function(sphere_){
  dbGetQuery(conn = con, paste0("SELECT DISTINCT s.sha1, s.crawl_date, s.site, t.tag FROM tags t INNER JOIN sites s on t.site = s.sha1 WHERE t.tag = 'form' AND t.sphere = 'German' AND s.of_interest = true"))
}
...
get_sites_with_form(SPHERE_FOR_TABSET) %>% 
...
get_form_attr_info <- function(sphere, tag, attr){
  df <- dbGetQuery(conn = con, paste0("SELECT t.site, t.tag, t.attr, t.value, s.crawl_date, s.sphere, t.group FROM sites s INNER JOIN tags t ON t.site = s.sha1 WHERE s.sphere = '", sphere, "' AND t.tag = '", tag, "' AND t.attr = '", attr, "' AND s.of_interest = true")) 
...
get_first_lines_of_NAs <- function(sphere_){
  ...
  mutate(script_detected = re2_match(attr, SCRIPT_ENDINGS_TO_SEARCH_FOR, simplify = TRUE)[,1]) %>%
  filter(is.na(script_detected)) %>% select(crawl_date, attr, value, group, site) %>% head(100)
}
...
get_form_with_comment <- function(sphere_, tag){
  # print(sphere_)
  df <- dbGetQuery(conn = con, paste0("SELECT s.crawl_date, t.site as sha1, s.url, s.sphere, t.tag, s.site, t.attr, t.value, t.group, regexp_matches(t.value, '", COMMENTS_IN_TAGS, "') as matches FROM sites s INNER JOIN tags t ON t.site = s.sha1 WHERE s.sphere = '", sphere_, "' AND t.tag = '", tag, "' AND s.of_interest = true"))%>%
...
get_tags_printing <- function(sphere){
  ...
  df_counted_attrs <- dbGetQuery(conn = con, paste0("SELECT t.attr, COUNT(s.sha1) as counted_attrs FROM sites s INNER JOIN tags t ON t.site = s.sha1 WHERE s.sphere = '", sphere, "'AND t.tag = 'form' AND s.of_interest = true GROUP BY t.attr"))
  ...
  ggplot(., aes(x = value, y = reorder(attr, desc(order)), fill = cat, label = value)) +
  geom_col() +
...
df_current_sphere_with_comment %>%
  ...
  ggplot(., aes(x = year, y = site, fill = color_breaks, tooltip = paste0(counted, "\nsite: ", site))) +
...
get_df <- function(sphere_){
  ...
  df_all_form_tags <- dbGetQuery(conn = con, paste0("SELECT s.crawl_date, s.sphere, s.site, t.site as sha1, s.url, t.tag, t.attr, t.value, t.group FROM sites s INNER JOIN tags t ON t.site = s.sha1 WHERE s.sphere ='", sphere_, "' AND t.tag = 'form'"))
  ...
  df_all_forms_with_finding <- df_all_form_tags %>%
    mutate(matches = str_detect(value, "comment|komment")) %>% #View()
